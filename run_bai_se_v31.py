...
def run_trials(algo, n_trials, n_arms, mu_best, mu_sub, sigma, delta):

    all_outputs = []
    all_stop_times = []
    for trial_idx in range(n_trials):
        output, stop_time = algo(n_arms, mu_best, mu_sub, sigma, delta)
...
def se(n_arms, mu_best, mu_sub, sigma, delta, max_iter=100000):
    K = n_arms
    ...
    for t in range(max_iter):
        ...
        thres = np.sqrt(2 * np.log(3.3 * K * ((t+1) ** 4) / delta) / arm_nsamples)
...
def main():
    ...
    for idx, algo_name in enumerate(algo_names):
        ...
        trials_outputs, trials_stop_times = \
            run_trials(se, n_trials, n_arms, mu_best, mu_sub, sigma, delta)
