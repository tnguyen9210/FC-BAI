...
class FCDoublingSequentialHalvingNoReuse:
    def __init__(self, K, reuse=False, factor=2.0, divisor=2.0, seed=19):
        """
        K      : the number of arms.
        factor : the budget of i-th SH will be `factor` times that of (i-1)-th SH. 
                 E.g., factor=2 means doubling.
...
def run_bandit_lucb_p(algo, env, delta, max_iter, sigma_sq = 1.0):
    table = KjTable() 
    b_stopped = False
    for t in range(max_iter):
        i_t = algo.next_arm()
        ...
        hatmu = algo.get_empirical_means()
        sidx = np.argsort(hatmu)[::-1]
        i_top = sidx[0]
        i_bot = sidx[1:]

        bonuses = algo.dev(algo.n_pulls, algo.delta/algo.K, algo.sig_sq, t)
        
        #- smallest LCB from the top
        h_t = i_top
        bar_LCB = hatmu[i_top] - bonuses[i_top]
        
        #- highest UCB from the bottom
        v = hatmu[i_bot] + bonuses[i_bot]
        maxv = v.max()
        idx = ra.choice(np.where(v == maxv)[0])
        ell_t = i_bot[idx]
        bar_UCB = maxv
...
def run_bandit_lucb_v21(algo, env, delta, max_iter, sigma_sq = 1.0, n_rigged = 5):
    table = KjTable() 
    b_stopped = False
    for t in range(max_iter):
        i_t = algo.next_arm()
...
def run_bandit_pe_v31(algo, env, delta, max_iter, sigma_sq = 1.0):
    table = KjTable() 
    b_stopped = False
    for t in range(max_iter):
        logging.info(f"\n->t = {t}")
